...
GPU_REQUIREMENTS["unsloth/Qwen3-32B-GGUF:Q8_K_XL"]="2"
DEFAULT_NUM_GPUS=1
...
for MODEL_PATH in "${MODEL_PATHS[@]}"; do
    for QUANT_MODE in "${QUANT_MODES[@]}"; do
        ...
        sed -i "s|^quant_mode:.*|quant_mode: $QUANT_MODE|" "$CONFIG_FILE"
        sed -i "s|^inference_backend:.*|inference_backend: llama-cpp|" "$CONFIG_FILE"
        echo "Updated '$CONFIG_FILE'."

        # Run the benchmark script
        echo "Running benchmark script..."
        ./scripts/run_benchmark.sh -g "$NUM_GPUS"
        echo "Benchmark job sent for $MODEL_PATH ($QUANT_MODE)."
